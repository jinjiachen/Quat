...
def Menu():
    ...
    if choice=="1":
        ...
        filename=freq+'suppress'+mas+'_'+n+"_"+m+'_'+now+'loc_new.txt' #文件名
        SaveResult(filename,result) #保存结果
    elif choice=='3':
        freq=input("请输入均线周期:")
        ma_s=input("请输入均线:")
...
def Suppress(freq,mas,n,m): #K线站上均线模型
    print('调取数据库')
    if freq=="D":
        sql="SELECT DISTINCT ts_code,trade_date,close FROM Daily ORDER BY trade_date DESC;" #构建SQL查询语句
    elif freq=="W":
        sql="SELECT DISTINCT ts_code,trade_date,close FROM Weekly ORDER BY trade_date DESC;" #构建SQL查询语句
    elif freq=="M":
        sql="SELECT DISTINCT ts_code,trade_date,close FROM Monthly ORDER BY trade_date DESC;" #构建SQL查询语句
    else:
        raise ValueError('查询周期错误')
    df=pd.read_sql_query(sql, engine_ts) #运用pandas模块read_sql_query方法执行SQL语句
    print('完成调取数据库')
...
#            print('len:'+str(len(data)))
        close=data['close']  #获取数据中close列
        close=function.Str_Float(close) #把数据转换成float
        ma=function.Average(close,mas,n)
#        pdb.set_trace()
        if ma==None:
            print('ma_error')
            continue
...
#        ma=data['ma'+str(ma_s)] #提取均线
        close=data['close']  #获取数据中close列
        close=function.Str_Float(close) #转换数据为Float
        ma=function.Average(close,ma_s,n) #计算均线数据
        if ma==None:#判断是否有空值
            continue
        elif len(ma)<n: #判断均线是否大于周期
            continue
        j=0 #初始化
...
#                pdb.set_trace()
                j=j+1
                if j+1>=max(ma_s,n):#当超过均线或者周期长度时，退出循环
                    break
            point=j #记录拐点
        else:
            continue
...
#def filter(ts_code):#对股票进行指定的过滤
#    sql='SELECT * FROM stock_basic where ts_code="'+ts_code+'" limit 1;' #构建SQL查询语句, 查询有重复，加limit 1取其中一个
#    df=pd.read_sql_query(sql, engine_ts) #运用pandas模块read_sql_query方法执行SQL语句
#    if 'ST' in df['name']:#去除ST的股票
#        return continue 
